package net.isabel.aeroscapes.world.level.levelgen.cluster;
...
public class BasaltClusterFeature extends Feature<DripstoneClusterFeatureConfig> {
    public BasaltClusterFeature(Codec<DripstoneClusterFeatureConfig> codec) {
        super(codec);
    }
    ...
    private class Helper {
        ...
        public static boolean canReplace(BlockState state) {
            return state.isOf(Blocks.BASALT) || state.isIn(BlockTags.DRIPSTONE_REPLACEABLE_BLOCKS);
        }

        private static BlockState getState(Direction direction, Thickness thickness) {
            return AeroscapesBlocks.POINTED_BASALT.getDefaultState().with(PointedDripstoneBlock.VERTICAL_DIRECTION, direction).with(PointedDripstoneBlock.THICKNESS, thickness);
        }
        ...
        protected static void getBasaltThickness(Direction direction, int height, boolean merge, Consumer<BlockState> callback) {
            if (height >= 3) {
        ...
        protected static void generatePointedBasalt(WorldAccess world, BlockPos pos, Direction direction, int height, boolean merge) {
            if (canReplace(world.getBlockState(pos.offset(direction.getOpposite())))) {
                BlockPos.Mutable mutable = pos.mutableCopy();
                getBasaltThickness(direction, height, merge, state -> {
                    if (state.isOf(AeroscapesBlocks.POINTED_BASALT)) {
    ...
    public boolean generate(FeatureContext<DripstoneClusterFeatureConfig> context) {
        ...
        if (!Helper.canGenerate(structureWorldAccess, blockPos)) {
            return false;
        } else {
    ...
    private void generate(
        ...
        if (!optional.isEmpty()) {
            ...
            if (!optionalInt.isEmpty() || !optionalInt2.isEmpty()) {
                ...
                if (optionalInt.isPresent()) {
                    Helper.generatePointedBasalt(world, pos.withY(optionalInt.getAsInt() - 1), Direction.DOWN, m, bl4);
                }

                if (optionalInt3.isPresent()) {
                    Helper.generatePointedBasalt(world, pos.withY(optionalInt3.getAsInt() + 1), Direction.UP, t, bl4);
                }
            }
        }
    }
    ...
    private void placeDripstoneBlocks(StructureWorldAccess world, BlockPos pos, int height, Direction direction) {
        ...
        for (int i = 0; i < height; i++) {
            if (!Helper.generateDripstoneBlock(world, mutable)) {
                return;
            }
